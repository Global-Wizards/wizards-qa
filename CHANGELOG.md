...
### Changed
- **Overview tab shows all screenshots** — The Overview tab now displays all captured screenshots (loading, after click, after button) in a responsive grid instead of only the first one. Click any screenshot to expand it in a dialog.
...
### Fixed
- **Exploration summary caused consecutive user messages** — The summary injection appended a new user message after tool results (also a user message), violating Claude API's role alternation requirement. Now merges summary as a text block into the preceding tool results message via `appendTextToLastUserMessage`.
...
### Fixed
- **Build break: undefined `normalizeFlowYAML`** — `server.go` still called the local `normalizeFlowYAML` wrapper removed in v0.40.0; replaced with `flows.NormalizeFlowYAML`. This broke CI deploy since v0.40.0.
