...
func (g *Generator) generateMarkdown(results *maestro.TestResults, gameName string) (string, error) {
	...
	md.WriteString(fmt.Sprintf("- **Timeout:** ⏱️  %d\n", results.Timeout))
	...
	if results.SuccessRate() == 100 {
		md.WriteString("✅ All tests passed! The game is working as expected.\n\n")
	} else if results.SuccessRate() >= 80 {
		md.WriteString("⚠️  Most tests passed, but some issues were found. Review failed tests above.\n\n")
	} else if results.SuccessRate() >= 50 {
		md.WriteString("⚠️  Significant issues detected. Multiple test failures need attention.\n\n")
	} else {
		md.WriteString("❌ Critical issues detected. Most tests are failing. Immediate action required.\n\n")
	}

	// Footer
	md.WriteString("---\n\n")
	md.WriteString("*Generated by Wizards QA* 🧙‍♂️\n")
...
func getStatusEmoji(results *maestro.TestResults) string {
	...
	if results.Failed > 0 || results.Timeout > 0 {
		return fmt.Sprintf("❌ %d/%d tests failed", results.Failed+results.Timeout, results.Total)
	}
